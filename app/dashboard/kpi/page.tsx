...
async function getSheetData(sheet_id: string) {
  if (!sheet_id) {
    ...
    return { label: 'Error', value: [], refreshedAt: null }
  }

  const cachedData = await cache(
    async () => {
      console.log(`FETCHING FRESH DATA for sheet: ${sheet_id}`)
      try {
        ...
        const refreshedAt = new Date().toISOString() // <-- 2. Create timestamp
          ...
          {sheetsWithKpi
            ...
            .map((item) => (
              <KPICard
                ...
                title={item.title}
                kpi={item.kpi.value}
                refreshedAt={item.kpi.refreshedAt ?? ''}
              />
            ))}
        </div>
      </section>
