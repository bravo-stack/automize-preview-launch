import { getFacebookAggregates } from '@/lib/services/data-hub'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const aggregates = await getFacebookAggregates(snapshotId)
    return NextResponse.json({ success: true, data: aggregates })
  } catch (error) {
    console.error('Error fetching Facebook aggregates:', error)
